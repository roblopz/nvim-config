local M = {
  excludeWinFileTypes = { 'NvimTree', "neo-tree", "notify", "TelescopePrompt", "qf", "dap-repl", "quickfix" }
}
...
local function get_pick_target_win_count()
  ...
  local tab_wins = vim.api.nvim_tabpage_list_wins(vim.fn.tabpagenr())

  for _, id in ipairs(tab_wins) do
    local win_bufnr = vim.api.nvim_win_get_buf(id)

    -- Check is not excluded and it's not floating
    if not vim.tbl_contains(M.excludeWinFileTypes, vim.api.nvim_buf_get_option(win_bufnr, 'filetype')) and
        vim.api.nvim_win_get_config(id).relative == "" then
