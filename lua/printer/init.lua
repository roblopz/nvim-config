-- Modified from https://github.com/rareitems/printer.nvim
...
local function get_text_from_textobject()
	...
	if range.srow == range.erow then
		...
		return vim.api.nvim_buf_get_text(0, range.srow - 1, range.scol, range.erow - 1, range.ecol + 1, {})[1]
	else
...
local function get_printer()
	local filetype = vim.bo.filetype
	local variant = vim.b["variant"]

	if variant ~= nil and variant ~= "default" then
		filetype = filetype .. "_" .. variant
	end

	return vim.b["printer"]
